...
- **Ocultação e Visualização da Barra Lateral do Módulo:** Implementado um botão interativo de alternância (Menu) no cabeçalho dos módulos de Follow-up para ocultar e reexibir a barra lateral interna do módulo de forma dinâmica. Isso maximiza o espaço visual de trabalho na tabela de solicitações, especialmente em telas compactas.
...
- **Remoção de Módulo de Acesso (Julho de 2026)**: Conclusão da remoção limpa do módulo "Controle de Acesso e Usuários" de todas as telas, rotas e painéis centrais do sistema, mantendo a arquitetura leve.
- **Otimização de Espaço no Follow-Up (Julho de 2026)**: Adição do mecanismo de ocultar/visualizar a barra lateral do módulo Follow-up com botão do tipo "Menu", para maximizar a área de trabalho do operador de produção.
- **Reorganização Estrutural do Menu Lateral (Julho de 2026)**: Agrupamento inteligente das telas em três sanfonas (accordions) expansíveis ("Controle de Reposição", "Plano de corte" e "Compras"), limpando o excesso visual do painel e mantendo "Chat" e "Configuração" como links diretos raiz para uma navegação de alto desempenho.
- **Granularidade Absoluta no Controle de Permissões (Julho de 2026)**: Expandido o módulo "Controle de Acesso e Usuários" de 7 para todos os 11 módulos do sistema. Cada tela ("Painel", "Cadastro", "Almox", "Entrega do Almox", "Entrega Dublagem", "Disponível na Central", "Produção", "Programação PCP", "Follow-up", "Chat" e "Configuração") passa a possuir um controle de permissão checkbox independente na tabela de configuração, mantendo compatibilidade reversa automática e suporte a fallbacks inteligentes para usuários existentes.
- **Renomeação Amigável de Módulo (Julho de 2026)**: Renomeada a tela e identificadores visuais de "Cadastro Entrega" para "Entrega do Almox", tornando o fluxo de expedição de insumos muito mais claro para os operadores do almoxarifado.
- **Tabela Dinâmica Tabular e Coluna de Observação Direta (Julho de 2026)**: Reformulada a exibição de agrupamentos na Tabela Dinâmica de Matéria-Prima (`MateriaPrimaPivotTable.tsx`). Substituída a árvore hierárquica aninhada por colunas de visualização tabular direta (Documento, Produto, Descrição do Produto, etc.), além de adicionar uma coluna de "Observação (pode escrever)" diretamente no grid com campos editáveis de input estilo planilha. Implementado tratamento de chaves únicas por `groupKey` composto para evitar conflitos de renderização no React.
